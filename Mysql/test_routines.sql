...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `car` AS select `shopcar`.`id` AS `id`,`member`.`memberId` AS `memberId`,`porder`.`id` AS `porderId`,`member`.`username` AS `username`,`member`.`name` AS `name`,`porder`.`product` AS `product`,`porder`.`amount` AS `amount`,`shopcar`.`num` AS `num`,`shopcar`.`sum` AS `sum`,`porder`.`mode` AS `mode` from ((`shopcar` join `member`) join `porder`) where ((`shopcar`.`memberId` = `member`.`memberId`) and (`shopcar`.`porderId` = `porder`.`id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-02-11 12:24:10
